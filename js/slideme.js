(function ($) {
        ...
        init: function (params) {
            ...
            return this.each(function () {
                ...
                if (options.itemsForSlide > 1) {
                    var slides = $('.slideme', this).children();
                    $('.slideme', this).children().addClass('itemsForSlide').css({ 'width': 100 / options.itemsForSlide + '%' });
                ...
                $(this).data({ 'current': 0, 'slides': calculate.childrens(this), 'direction': 1, 'speed': options.speed, 'ready': true, 'transition': 'slideme-'+options.transition }).addClass('slideme_container');
                ...
                if (options.resizable.width && options.resizable.height) {
                    $(this).data({ 'resizable': true, 'width': options.resizable.width, 'height': options.resizable.height }).addClass('resizable').find('.slideme').css({ 'height': calculate.height(this) });
                }
    ...
    var events = {
        onPaginationClicked: function (event) {
            var target = $(event.currentTarget).parents('.slideme_container');
        ...
        onResize: function () {
            $(".resizable").each(function (index) {
                $('.slideme', this).css({ 'height': calculate.height(this) }).stop(true, true);
            });
        }
    } // end events.
    ...
    var calculate = {
        childrens: function (target) {
            return $('.slideme', target).children().length - 1;
        },
        height: function (target) {
            return $(target).data('height') * $('.slideme', target).width() / $(target).data('width');
        },
    ...
    var animate = {
        playTo: function (target, index, direction) {
            ...
            if ($(target).data('transition') == "fade") {
                ...
                if ($(target).data('css3')) {
                    ...
                    $('.slideme', target).children('.current').css({ 'z-index': 300 }).animate({
                        'opacity': 0
                ...
                $('.slideme', target).children().eq(index).addClass('next').attr({ 'style': 'left: ' + direction * '100' + '%;' });
                if ($(target).data('css3')) {
                    $(target).addClass($(target).data('transition'));
                    $('.slideme .next', target).show().attr({ 'style': 'left: 0%;' }).on('otransitionend transitionend webkitTransitionEnd', function () {
                        $(this).off('otransitionend transitionend webkitTransitionEnd');
                        comands.onEndSlide(target, index);
                    });
                } else {
                    $('.slideme .next', target).css({ 'z-index': 300 }).animate({
                        'left': '0%'
                    }, $(target).data('speed'), function () {
                        comands.onEndSlide(target, index);
                    });
                ...
                if ($(target).data('css3')) {
                    $(target).addClass($(target).data('transition'));
                    $('.slideme .current', target).show().attr({ 'style': 'left: ' + (-direction * '100' + '%;') });
                    $('.slideme .next', target).show().attr({ 'style': 'left: 0%;' }).on('otransitionend transitionend webkitTransitionEnd', function () {
                        $(this).off('otransitionend transitionend webkitTransitionEnd');
                        comands.onEndSlide(target, index);
                    });
                } else {
                    $('.slideme .current', target).animate({
                        'left': direction === 1 ? '-100%' : '100%'
                    }, $(target).data('speed'));
                    $('.slideme .next', target).animate({
                        'left': '0%'
                    }, $(target).data('speed'), function () {
                        comands.onEndSlide(target, index);
                    });
        ...
        jumpTo: function (target, index) {
            ...
            $('.slideme', target).children().eq(index).addClass('next').css({ 'left': 0, 'opacity': 1, 'z-index': 100 });
            comands.onEndSlide(target, index);
        }
    } // end animate.
    ...
    var comands = {
        setCurrent: function (target, index) {
            $('.slideme', target).children().removeClass('current').eq(index).addClass('current');
        ...
        onEndSlide: function (target, index) {
            ...
            $('.slideme', target).children().removeClass('current next');
        ...
        update: function (target) {
            ...
            $('.slideme', target).children().eq(0).addClass('next').css({ 'left': 0, 'opacity': 1, 'z-index': 100 });
    ...
    var init = {
        numbers: function (target) {
            ...
            var numbers = $('<ol />');
            $('.slideme', target).children().each(function (index) {
